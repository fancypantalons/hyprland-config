---
-- Rofi-based Helper Functions
-- Provides rofi menus for music, calculator, emoji picker, search, theme selection, and animations
--
-- All functions use baked-in values and show notifications via the notify module
--
-- @module user-functions.rofi
-- @author Brett
-- @license MIT
...
local ICON_DIR = CONFIG_DIR .. "/swaync/icons"
local SCRIPTS_DIR = HYPR_DIR .. "/scripts"
local USER_CONFIGS = HYPR_DIR .. "/UserConfigs"

-- Rofi theme files
local ROFI_THEME_BEATS = ROFI_DIR .. "/config-rofi-Beats.rasi"
local ROFI_THEME_BEATS_MENU = ROFI_DIR .. "/config-rofi-Beats-menu.rasi"
local ROFI_THEME_CALC = ROFI_DIR .. "/config-calc.rasi"
local ROFI_THEME_EMOJI = ROFI_DIR .. "/config-emoji.rasi"
local ROFI_THEME_SEARCH = ROFI_DIR .. "/config-search.rasi"
local ROFI_THEME_ANIMATIONS = ROFI_DIR .. "/config-Animations.rasi"
local ROFI_THEME_SELECTOR = ROFI_DIR .. "/config-rofi-theme.rasi"
local ROFI_CONFIG_FILE = ROFI_DIR .. "/config.rasi"

-- Theme directories
local ROFI_THEMES_DIR_CONFIG = ROFI_DIR .. "/themes"
local ROFI_THEMES_DIR_LOCAL = HOME .. "/.local/share/rofi/themes"
...
-- ============================================
-- EMOJI DATA (from RofiEmoji.sh)
-- ============================================

local EMOJI_DATA = [[😀 grinning face face smile happy joy :D grin
...
---Check if mpv is currently playing music (not mpvpaper)
-- @return boolean True if mpv is running
local function is_music_playing()
    local result = helpers.exec("pgrep -x mpv")
    ...
    if (mpv_pids == "" or mpv_pids == nil) then
        return false
    end

    -- Check for mpvpaper process to exclude it
...
---Stop all mpv music playback (excluding mpvpaper)
local function stop_music()
    local result = helpers.exec("pgrep -x mpv")

    if (not result.success or result.stdout == "" or result.stdout == nil) then
        return
    end
...
---Send a music notification
-- @param message string The message to display
local function notify_music(message)
    notify.send({
        text = "Now Playing: " .. message,
        icon = ICON_DIR .. "/music.png",
        timeout = 3000
    })
end

---Kill any running rofi instance
local function kill_rofi()
    hl.exec_cmd("pkill -x rofi 2>/dev/null || true")
end

---Get the Search_Engine from UserDefaults config
-- @return string|nil The search engine URL or nil
local function get_search_engine()
    local result = helpers.exec("grep 'Search_Engine' " .. USER_CONFIGS .. "/01-UserDefaults.conf 2>/dev/null | head -1")
    ...
    local engine = string.match(result.stdout, "=%s*(.+)")

    if (engine) then
        -- Remove quotes if present
        engine = string.gsub(engine, "^\"", "")
        engine = string.gsub(engine, "\"$", "")
        engine = string.gsub(engine, "^'", "")
        engine = string.gsub(engine, "'$", "")
        -- Trim whitespace
...
-- Displays a searchable list of emojis in rofi.
...
-- Opens a rofi input that searches using the configured search engine.
...
-- Lists animation files from ~/.config/hypr/animations/
-- Copies selected to UserConfigs/UserAnimations.conf and runs internal refresh (no waybar)
-- @function animations
function rofi.animations()
    local success, err = pcall(function()
        ...
        local anim_result = helpers.exec(string.format(
            "find -L '%s' -maxdepth 1 -type f -name '*.conf' | sed 's/.*\\///' | sed 's/\\.conf$//' | sort -V",
            ANIMATIONS_DIR
        ))

        if (not anim_result.success or anim_result.stdout == nil or anim_result.stdout == "") then
            notify.error("No animation files found")

            return
        end

        -- Parse animations into array
        local animations = {}

        for line in string.gmatch(anim_result.stdout, "[^\n]+") do
            if (line ~= "" and line ~= nil) then
        ...
        local msg = "❗NOTE:❗ This will copy animations into UserAnimations.conf"
        local anim_list = table.concat(animations, "\n")

        local rofi_result = helpers.exec(string.format(
            "echo '%s' | rofi -i -dmenu -config %s -mesg '%s'",
            anim_list,
            ROFI_THEME_ANIMATIONS,
            msg
        ))

        if (not rofi_result.success or rofi_result.stdout == nil or rofi_result.stdout == "") then
            return
        end

        local chosen = string.gsub(rofi_result.stdout, "%s+$", "")

        if (chosen == "") then
            return
        end

        local source_file = ANIMATIONS_DIR .. "/" .. chosen .. ".conf"
        local dest_file = USER_CONFIGS .. "/UserAnimations.conf"

        -- Copy the file
        local copy_result = helpers.exec(string.format("cp '%s' '%s'", source_file, dest_file))

        if (not copy_result.success) then
            notify.error("Failed to copy animation file")

            return
        end

        notify.send({
            text = chosen .. " - Hyprland Animation Loaded",
            icon = ICON_DIR .. "/ja.png",
            timeout = 2000
        })
